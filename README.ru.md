...
> **Обновляйте обе стороны одновременно.** В этой сборке каждый пакет
> помечается однобайтовым фреймом, чтобы keep-alive проходил через то же
> сжатие и шифрование, что и обычный трафик, а не был постоянным маркером в
> открытом виде. Сторона на старой сборке этот байт не понимает, поэтому
> клиент и выходную ноду нужно обновлять вместе.
